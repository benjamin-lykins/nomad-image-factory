...
locals {
  build_source = lookup(local.sources, var.platform, ["source.amazon-ebs.nginx"])
  sources = {
    aws     = ["source.amazon-ebs.nginx"]
    azure   = ["source.azure-arm.nginx"]
    vsphere = ["source.vsphere-clone.nginx"]
  }
}
...
build {
  ...
  provisioner "ansible" {
    ...
    playbook_file          = "${path.cwd}/packer/playbooks/app-nginx-playbook.yml"
