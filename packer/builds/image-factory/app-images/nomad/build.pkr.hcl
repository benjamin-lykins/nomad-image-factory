...
variable "nomad_role" {
  type        = string
  description = "The list of sources to build."
  default     = env("NOMAD_ROLE")
}

locals {
  build_source = lookup(local.sources, var.platform, ["source.amazon-ebs.nomad-server"])
  sources = {
    aws     = ["source.amazon-ebs.${var.nomad_role}"]
    azure   = ["source.azure-arm.${var.nomad_role}"]
    vsphere = ["source.vsphere-clone.${var.nomad_role}"]
  }
}
...
build {
  ...
  provisioner "ansible" {
    ...
    playbook_file = "${path.cwd}/packer/playbooks/app-nomad-playbook.yml"
    ...
    extra_arguments = [
      ...
      "--extra-vars", "nomad_role='${source.name}'",
  ...
  provisioner "shell" {
    only = [
      "azure-arm.nomad-client",
      "azure-arm.nomad-server",
      "azure-arm.nomad-shared"
    ]
