...
locals {
  build_source = lookup(local.sources, var.platform, ["source.amazon-ebs.ubuntu"])
  sources = {
    aws     = ["source.amazon-ebs.ubuntu"]
    azure   = ["source.azure-arm.ubuntu"]
    vsphere = ["source.vsphere-iso.ubuntu"]
  }
}
...
build {
  ...
  provisioner "ansible" {
    ...
    playbook_file          = "${path.cwd}/packer/playbooks/base-linux-playbook.yml"
